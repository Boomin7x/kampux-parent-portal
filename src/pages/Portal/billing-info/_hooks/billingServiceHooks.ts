...
const useGetBillingsByRegistration = ( registrationId: number) => {
    return useQuery({
        queryKey: [billingServiceQueryKeys.getBillingsByRegistration, registrationId],
...
const useCreateBilling = ( ) => {
    ...
    return useMutation({
        ...
        onSuccess: () => {
            [billingServiceQueryKeys.getBillings, billingServiceQueryKeys.getBilling].forEach((key) => {
...
const useUpdateBilling = () => {
    ...
    return useMutation({
        ...
        mutationFn: (billing: IBilling) => billingService.updateBilling(billing),
        onSuccess: () => {
            [billingServiceQueryKeys.getBillings,  billingServiceQueryKeys.getBilling].forEach((key) => {
                queryClient.invalidateQueries({ queryKey: [key] })
            })
...
export { useCreateBilling, useGetBilling, useGetBillings, useGetBillingsByRegistration, useUpdateBilling }
